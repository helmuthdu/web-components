...
const getClassNames = (props: Props & HTMLElement) => {
  return pickClassNames(
    ...
    hasProp(props.rounded) ? 'rounded-full' : 'rounded-lg',
    hasProp(props.circle)
      ? {
    ...
    hasProp(props.block) && 'w-full',
    hasProp(props.disabled)
      ...
      ? {
          ...
          'text-orange-500 hover:text-orange-600 border-orange-500 hover:border-orange-600': props.variant === 'orange',
          'text-amber-500 hover:text-amber-600 border-amber-500 hover:border-amber-600': props.variant === 'amber',
          'text-yellow-500 hover:text-yellow-600 border-yellow-500 hover:border-yellow-600': props.variant === 'yellow',
          ...
          'text-indigo-500 hover:text-indigo-600 border-indigo-500 hover:border-indigo-600': props.variant === 'indigo',
          'text-violet-500 hover:text-violet-600 border-violet-500 hover:border-violet-600': props.variant === 'violet',
          'text-purple-500 hover:text-purple-600 border-purple-500 hover:border-purple-600': props.variant === 'purple',
...
export default define<Props>({
  ...
  render: props =>
    html`
      <button type="${props.type}" class="${getClassNames(props)}">
        ${props.loading
          ? html`
              <svg
                id="loading"
                class="${pickClassNames(
                  'animate-spin',
                  { 'mr-2': !hasProp(props.circle), [`text-${props.variant}`]: props.variant },
                  {
